  ...
  private _registerAllRoutes(): void {
    ...
    this._registerRoute(
      ...
      async (req): Promise<SyncingResponse> => {
        ...
        const highestL2BlockNumber = await db.getHighestL2BlockNumber()
        const currentL2Block = await db.getLatestTransaction()
    ...
    this._registerRoute(
      ...
      async (req): Promise<TransactionResponse> => {
        const chainId=BigNumber.from(req.params.chainId).toNumber()
        const db=await this._getDb(chainId)
        let transaction = await db.getLatestFullTransaction()
        if (this.options.showUnconfirmedTransactions) {
          const latestUnconfirmedTx = await db.getLatestUnconfirmedTransaction()
          if (
            transaction === null ||
    ...
    this._registerRoute(
      ...
      async (req): Promise<StateRootResponse> => {
        const chainId=BigNumber.from(req.params.chainId).toNumber()
        const db=await this._getDb(chainId)
        let stateRoot = await db.getLatestStateRoot()
        if (this.options.showUnconfirmedTransactions) {
          const latestUnconfirmedStateRoot = await db.getLatestUnconfirmedStateRoot()
          if (
            stateRoot === null ||
            stateRoot === undefined ||
